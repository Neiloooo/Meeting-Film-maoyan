...
public class DefaultAlipayServiceImpl implements AlipayServiceAPI {
    ...
    public AliPayResultVO getOrderStatus(String orderId) {
        ...
        //这里应该将userId作为条件,一起传入sql,进行用户与订单id的组合查询
        //但是这里就偷懒,只看看,服务端能不能接收到消费端传递进来的参数(userId)
        String userId = RpcContext.getContext().getAttachment("userId");
        log.info("用户的userId是="+userId);
       ...
       //初始化判断标识
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                //自己填写:当订单支付成功时,调用我们自己写的修改订单表接口,将状态改为1
                //并且通过中间字符flag判断,修改是否成功
    ...
    // 接口一调用的阿里接口:当面付2.0生成支付二维码
    public String trade_precreate (String orderId) {
        //返回地址,默认为空
        String filePath="";
        //根据订单编号获取订单信息,我们自己写的接口
        ...
        String body = "购买电影票一共花费"+orderVO.getOrderPrice();

        // 商户操作员编号，添加此参数可以为商户操作员做销售统计
        String operatorId = "操作一号";
...
//        goodsDetailList.add(goods2);
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                filePath = String.format("C:/Users/AK/Desktop/qrcode/qr-%s.png",
                        response.getOutTradeNo());
                //自定义文件姓名,这里使用的是二位码原姓名
                //String.format方法(%s,参数),其中可以参数会被传入到s中当字符串使用.小的方法
                String fileName = String.format("qr-%s.png",response.getOutTradeNo());
                log.info("filePath:" + filePath);

                //这个工具类除了帮我们生成二维码还可以帮我们返回文件流
